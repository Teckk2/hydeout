...
<br>So after the exploit completed sucessfully it created a new user and 2 new files (session.json) and (user.json)and if you look inside the session.json file you will see (session_ID, name and token) and we will edit and use this to gain admin access on the web.
